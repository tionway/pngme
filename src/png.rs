...
impl Png {
    ...
    pub fn remove_first_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        let idx = self
            .chunks
            .iter()
            .position(|chunk| chunk_type.eq(chunk.chunk_type()))
            .ok_or("No such chunk.".to_owned())?;
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> std::result::Result<Self, Self::Error> {
        ...
        if length < 8 {
            return Err(
                PngmeParseError::PngErr("A header of PNG file needs 8 bytes.".to_owned()).into(),
            );
        }

        if Png::STANDARD_HEADER != value[..8] {
            return Err(PngmeParseError::PngErr("Wrong header.".to_owned()).into());
        }

        fn chunk_length(bytes: &[u8]) -> Result<u32> {
            Ok(u32::from_be_bytes((*bytes).try_into()?))
        }

        let mut idx = 8;
        let mut chunks = Vec::with_capacity(length / 12);
        while idx <= length - 12 {
            let length = chunk_length(&value[idx..idx + 4])? as usize;
            chunks.push(Chunk::try_from(&value[idx..idx + 12 + length])?);
            idx += 12 + length;
        }
